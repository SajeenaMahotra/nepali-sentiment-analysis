...
This project builds and evaluates a sentiment classification pipeline on 4,343 real customer reviews scraped directly from Daraz Nepal, comparing three traditional machine learning classifiers trained on TF-IDF features against two fine-tuned multilingual transformers:

| Model | Macro F1 |
|---|---|
| XLM-RoBERTa (fine-tuned) | 0.69 |
| MuRIL (fine-tuned) | 0.68 |
| Logistic Regression | 0.65 |
| Support Vector Machine | 0.64 |
| Naive Bayes | 0.56 |

*The live demo above serves the three classical models. MuRIL and XLM-RoBERTa are trained, evaluated, and compared in the notebooks and accompanying paper, but are not part of the deployed web app.*
...
- Fine-tuned MuRIL and XLM-RoBERTa transformer models with class-weighted training to address neutral-class imbalance
...
**Modeling and data processing:** Python, pandas, NumPy, scikit-learn, Hugging Face Transformers, PyTorch, NLTK, Requests
...
Run the notebooks in `notebooks/` in numerical order (00 through 07) to reproduce the full pipeline from data collection to final evaluation and figures. Note: `00_data_collection.ipynb` describes the collection methodology only and is not intended to be re-run, since scraping the same live pages again would not reproduce the original dataset.
...
- Nepali morphological negation (for example "chaina" or "thiyena," where negation is fused into the verb rather than expressed as a separate word) is not well captured by the current features, and reviews using this pattern are more prone to misclassification.
